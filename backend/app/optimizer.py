"""Squad optimizer v1, per MODEL_SPEC.md section 3.

v1 scope: single-gameweek squad selection from scratch — build the best possible
15-man squad, starting XI, and captain for the upcoming gameweek. Multi-gameweek
horizon (v3) is out of scope here.
"""
...
def optimize_squad(players: list[OptimizerInput]) -> dict:
    ...
    prob += (
        pulp.lpSum(by_id[i].xp * start[i] for i in start)
    ...
    starting_ids = [i for i in start if start[i].value() == 1]
    captain_id = next(i for i in captain if captain[i].value() == 1)
